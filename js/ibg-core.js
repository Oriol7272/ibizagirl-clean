(function(){
  ...
  function detect(folder){
    ...
    const cands=[];for(const [k,v] of Object.entries(window)){if(Array.isArray(v)&&v.length){const f=v[0];if(typeof f==="string"&&isImgFilename(f)){cands.push(normalize(v,folder))}else if(typeof f==="object"){if(f&&(f.folder===folder||fromObj(f))){cands.push(normalize(v,folder))}}}}
    cands.sort((a,b)=>b.length-a.length);return cands[0]||[]
  }

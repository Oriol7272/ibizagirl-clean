(function(){
  ...
  function srcFrom(o){if(!o||typeof o!=="object")return null; return o.src||o.url||o.path||o.file||null}
  function withBase(folder,file){
    var b=(window.ENV&&window.ENV.IBG_ASSETS_BASE_URL)||"";
    if(!b) return folder+"/"+file;
    b=b.replace(/\/+$/,"");
    return b+"/"+folder+"/"+file;
  }
  function normalize(list, folder){
    const out=[]; if(!Array.isArray(list)) return out;
    for(const it of list){
      if(typeof it==="string" && isImg(it)){
        const src = (it.startsWith("http")||it.startsWith("/"))? it : withBase(folder,it);
        out.push({src})
      } else if (typeof it==="object"){
        const f = srcFrom(it);
        if(f){
          const src = (f.startsWith("http")||f.startsWith("/"))? f : withBase(folder,f);
          out.push({src})
        }
      }
    }
  ...
  // Preferencia: UnifiedContentAPI (según tu repo), luego heurística global
  ...
  function detect(folder){
    ...
    for(const [k,v] of Object.entries(window)){
      if(Array.isArray(v)&&v.length){
        const f=v[0];
        if(typeof f==="string"&&isImg(f)) cands.push(normalize(v,folder));
        else if(typeof f==="object"){ if(f&&(f.folder===folder||srcFrom(f))) cands.push(normalize(v,folder)); }
      }
    }
  ...
  function getFullList(){ return detect("full"); }          // content-data2
  function getUncensoredList(){ return detect("uncensored"); } // content-data3+4
  ...
  function loadAds(){
    try{
      ...
      const inject=(el,html)=>{ if(el && html){ el.innerHTML = html; } };
  ...
  function loadCrisp(){
    try{
      const id = window.ENV && window.ENV.CRISP_WEBSITE_ID;
      if(!id) return;
  ...
  window.IBG={shuffle,pickRandom,getFullList,getUncensoredList,loadAds,loadCrisp};
})();

(function(){
  ...
  function inject(id, url, ns){
    ...
    s.setAttribute("data-namespace", ns);  // 👈 atributo correcto (antes iba en la URL → 400)
  ...
  // Subscripciones (si hay algún plan)
  var needSubs = (ENV.PAYPAL_PLAN_ID_MONTHLY || ENV.PAYPAL_PLAN_ID_ANNUAL);
  if (needSubs){
    inject(
      "sdk-pp-subs",
      "https://www.paypal.com/sdk/js?client-id="+encodeURIComponent(CID)+"&components=buttons&currency=EUR&intent=subscription&vault=true",
      "pp_subs"
    );
  }
